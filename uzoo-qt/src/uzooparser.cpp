	...
	: QObject(parent,name)
{
	...
	recvMaxNum = 500; 	// Default value
	myPort = 9001;		// Default value
...
UzooParser::initSearchString(const QString &searchstring_)
{
	...
	memcpy(&(message.send_data[11]) , searchString.latin1() , 
			sizeof(uchar)*(searchString.length()));
...
UzooParser::slotRecvMessageFromClients()
{
	...
	emit nowRecvSendUDPClients(recvCounter,getClientsInfoSize());
	...
	/* 역시 단박에 디코딩 */
	transrate_decoding(b1, b2, ip1, ip2, ip3, ip4, 
			tempNum ,_ip1 , _ip2 , _ip3 ,_ip4 );
	clientAddress.sprintf("%d.%d.%d.%d", _ip1 , _ip2 , _ip3 , _ip4);
	clientMessage = QString( (char*) &string[11] );

	ResultSearch result; // 정보를 담아서 보내야한다.
	result.address = clientAddress;

	int fIndex; 
	QString getString;

	fIndex = clientMessage.find("\n");
	getString = clientMessage.left(fIndex);
	clientMessage.remove(0,fIndex+1); // 첫번째 줄을 지운다.
	...
	while(clientMessage.lower().contains(".mp3") > 0)
	{
		// 패스가 아닌 쓰레기값을 지운다.
		while(1)
		{
		...
		fIndex = 0;
		fIndex = clientMessage.find("\n\r");
		clientMessage.remove(0, fIndex + 2);
		
		// 리스트에 정보를 넣는다.
		ResultSearch sendResult;
		sendResult.address = result.address;
		sendResult.userid  = result.userid;
		sendResult.port    = result.port;
		sendResult.path	   = result.path;
		sendResult.filesize= result.filesize;
		sendResult.filetime= result.filetime;
		sendResult.bitrate = result.bitrate;
		sendResult.singer  = result.singer;
		sendResult.title   = result.title;
		sendResult.comment = result.comment;
		sendResult.downsize= 0;
		sendResult.state   = NODOWN;
		int pathFindLen = sendResult.path.length();
		int pathFindIndex = sendResult.path.findRev("\\");
		sendResult.filename= sendResult.path.right(pathFindLen - pathFindIndex -1 );
		emit sendClientMessage(sendResult);
	}
}
