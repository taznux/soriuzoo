...
UzooHelpImpl::UzooHelpImpl(QWidget*parent,const char*name,bool modal,
	int fl): UzooHelp(parent,name,modal,fl)
{
	browser = "konqueror";
	initText();
	initBrowser();

	connect( helpBrowserComboBox , SIGNAL(activated(int)),
		this , SLOT(slotSwitchHelpBrowser(int)));
}
UzooHelpImpl::~UzooHelpImpl()
{
}
void
UzooHelpImpl::initBrowser()
{
	slotSwitchHelpBrowser(helpBrowserComboBox->currentItem());
}
void
UzooHelpImpl::slotSwitchHelpBrowser(int num)
{
	switch(num)
	{
		case(0):
			helpLabel->setText(kor(
			"<center>소리우주 소스포지 홈페이지</center>"
			"<p>"
			"다양한 환경의 소리우주를 만나실수 있습니다."
			"</p>"));
			break;
		case(1):
			helpLabel->setText(kor(
			"<center>소리우즈 Qt 홈페이지</center>"
			"<p>"
			"Qt용 소리우즈 홈페이지입니다. 사용시 문제점들을 이메일이나"
			"게시판에 올려주세요."
			"</p>"
			));
			break;
		...
		case(3):
			helpLabel->setText(kor(
			"<center>소리바다 공식홈페이지</center>"
			"<p>"
			"소리바다와 소리우즈는 상업적으로 아무런 관계가 없습니다."
			"</p>"
			));
			break;
		default:
...
void
UzooHelpImpl::initText()
{
	develBrowser->setText(kor(
	"soriuzoo developer list<br>"
	"Analysis/Design<br>"
	"<b>장혜식</b> <a href=""mailto://perky@users.sourceforge.net>perky@users.sourceforge.net</a><br>"
	"Qt Developer<br>"
	"<b>이재일</b> <a href=""mailto://sonegy@hanmail.net"">sonegy@hanmail.net</a><br>"
	"A Lee , al22 , Packager , al22@users.sourceforge.net<br>"
	"Park joon cheol , exman , Developer , exman@users.sourceforge.net<br>"
	"DaeHyun Nam , imations , Developer , imations@users.sourceforge.net<br>"
	"Sejin Hwang , parting ,Developer , parting@users.sourceforge.net<br>"
	"Ru Yon , ruyon , Developer , ruyon@users.sourceforge.net<br>"
	"Wook-jin Choi , taznux , Developer , taznux@users.sourceforge.net<br>"
	"Jang-ho Hwang , xrath , Developer , xrath@users.sourceforge.net<br>"
	));
	
	licenseBrowser->setText(kor(
